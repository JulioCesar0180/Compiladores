...
class VisitorTipos(object):
            ...
            for stmt in aux:
                if isinstance(stmt, str):
                    ...
                    self.ast += '\t"Sentencia_Seleccion ' + str(id_sentencia_seleccion) + '" ->' + str(
                        self.id_nodo) + '\n'
                    self.ast += '\t"Lista_Sentencias 1' + '" ->' + '"Sentencia_Seleccion ' + str(
                        id_sentencia_seleccion) + '"\n'
                elif stmt is not None:
    ...
    def visit_expresion(self, expresion):
        ...
        if expresion.expresion1_p is not None:
            if not isinstance(expresion.expresion1_p, list):
                aux = [expresion.expresion1_p]
            else:
                aux = expresion.expresion1_p
        ...
        #Verificar si variable está en Globales
        for i in range (len(general.listGlobales)):
            if(general.listGlobales[i].ID == str(expresion.var_p)):
                if(general.listGlobales[i].def_tipo == 'ENT'):
                    if (general.listGlobales[i].esArreglo == False):
                        print('Variable ' + str(expresion.var_p) + ' declarada como Global')
                    elif (general.listGlobales[i].esArreglo == True):
                        print('Error')
                elif (general.listGlobales[i].def_tipo == 'VACUO'):
                    print('Error')

        #Verificar si variable está en Parámetros
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listParam)):
                if(general.listFuncion[i].listParam[j].ID == str(expresion.var_p)):
                    if (general.listFuncion[i].listParam[j].def_tipo == 'ENT'):
                        if(general.listFuncion[i].listParam[j].esArreglo == False):
                            print('Variable ' + str(expresion.var_p) + ' declarada como Parametro')
                        elif(general.listFuncion[i].listParam[j].esArreglo == True):
                            print('Error')
                    elif (general.listFuncion[i].listParam[j].def_tipo == 'VACUO'):
                        print('Error')

        # Verificar si variable está en Locales
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listLocales)):
                if (general.listFuncion[i].listLocales[j].ID == str(expresion.var_p)):
                    if (general.listFuncion[i].listLocales[j].def_tipo == 'ENT'):
                        if(general.listFuncion[i].listLocales[j].esArreglo == False):
                            print('Variable ' + str(expresion.var_p) + ' declarada como Local')
                        elif(general.listFuncion[i].listLocales[j].esArreglo == True):
                            print('Error')
                    elif (general.listFuncion[i].listLocales[j].def_tipo == 'VACUO'):
                        print('Error')
        ...
        # Verificar si variable está en Globales
        for i in range(len(general.listGlobales)):
            if (general.listGlobales[i].ID == str(expresion.expresion1_p)):
                if (general.listGlobales[i].def_tipo == 'ENT'):
                    if (general.listGlobales[i].esArreglo == False):
                        print('Variable ' + str(expresion.expresion1_p) + ' declarada como Global')
                    elif (general.listGlobales[i].esArreglo == True):
                        print('Error')
                elif (general.listGlobales[i].def_tipo == 'VACUO'):
                    print('Error')
        ...
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listParam)):
                if (general.listFuncion[i].listParam[j].ID == str(expresion.expresion1_p)):
                    if (general.listFuncion[i].listParam[j].def_tipo == 'ENT'):
                        if(general.listFuncion[i].listParam[j].esArreglo == False):
                            print('Variable ' + str(expresion.expresion1_p) + ' declarada como Parametro')
                        elif(general.listFuncion[i].listParam[j].esArreglo == True):
                            print('Error')
                    elif (general.listFuncion[i].listParam[j].def_tipo == 'VACUO'):
                        print('Error')
        ...
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listLocales)):
                if (general.listFuncion[i].listLocales[j].ID == str(expresion.expresion1_p)):
                    if (general.listFuncion[i].listLocales[j].def_tipo == 'ENT'):
                        if(general.listFuncion[i].listLocales[j].esArreglo == False):
                            print('Variable ' + str(expresion.expresion1_p) + ' declarada como Local')
                        elif(general.listFuncion[i].listLocales[j].esArreglo == True):
                            print('Error')
                    elif (general.listFuncion[i].listLocales[j].def_tipo == 'VACUO'):
                        print('Error')

        if(str(expresion.expresion1_p).isnumeric()):
            print(str(expresion.expresion1_p))

        print(str(expresion.var_p)+" = "+str(expresion.expresion1_p))
    ...
    def visit_var(self, var):
        ...
        self.id_nodo += 1
        self.ast += '\t' + str(self.id_nodo) + '[label="' + str(var.ID_t) + '"]\n'


        print("=====================================================")
        print(isinstance(var.expresion1_p, str))
        print(str(var.expresion1_p))
        print("=====================================================")

        if isinstance(var.expresion1_p, str):
            self.ast += '-> "Var ' + str(id_var) + '" -> ' + str(self.id_nodo) + '"\n'
            self.id_nodo += 1
            self.ast += '\t' + str(self.id_nodo) + '[label="' + str(var.expresion1_p) + '"]\n'
            self.ast += '-> "Var ' + str(id_var) + '" -> ' + str(self.id_nodo) + '"\n'
        else:
            self.ast += '-> "Var ' + str(id_var) + '"'
            var.expresion1_p.accept(self)
    ...
    def visit_sentencia_retorno(self, sentencia_retorno):
        ...
        for i in range(len(general.listGlobales)):
            if (general.listGlobales[i].ID == str(sentencia_retorno.expresion_p)):
                if (general.listGlobales[i].def_tipo == 'ENT'):
        ...
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listParam)):
                if (general.listFuncion[i].listParam[j].ID == str(sentencia_retorno.expresion_p)):
                    if (general.listFuncion[i].listParam[j].def_tipo == 'ENT'):
                        print('Retorno Corecto')
                    elif (general.listFuncion[i].listParam[j].def_tipo == 'VACUO'):
                        print('Error')
        ...
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listLocales)):
                if (general.listFuncion[i].listLocales[j].ID == str(sentencia_retorno.expresion_p)):
                    if (general.listFuncion[i].listLocales[j].def_tipo == 'ENT'):
                        print('Retorno Correcto')
                    elif (general.listFuncion[i].listLocales[j].def_tipo == 'VACUO'):
                        print('Error')

        if (str(sentencia_retorno.expresion_p).isnumeric()):
            print('Retorno Correcto')
    ...
    def visit_sentencia_iteracion1(self, sentencia_iteracion):
        ...
        print('AAAHHH' + str(sentencia_iteracion.expresion_p))
        ...
        if sentencia_iteracion.sentencia_p is not None:
            ...
            for stmt in aux:
                if isinstance(stmt, str):
                    self.id_nodo += 1
                    self.ast += '\t' + str(self.id_nodo) + '[label="' + stmt + '"]\n'
                    self.ast += '->"Sentencia_Iteracion ' + str(id_sentencia_iteracion) + '" ->' + str(
                        self.id_nodo) + '\n'
                elif stmt is not None:
                    self.ast += '\t"Sentencia_Iteracion ' + str(id_sentencia_iteracion) + '"'
                    stmt.accept(self)
    ...
    def visit_expresion_simple(self, expresion_simple):
        ...
        #Verificar si variable está en Globales
        for i in range (len(general.listGlobales)):
            if(general.listGlobales[i].ID == str(expresion_simple.expresion_simple_p)):
                if(general.listGlobales[i].def_tipo == 'ENT'):
                    if (general.listGlobales[i].esArreglo == False):
                        print('Variable ' + str(expresion_simple.expresion_simple_p) + ' declarada como Global')
                    elif (general.listGlobales[i].esArreglo == True):
                        print('Error')
                elif (general.listGlobales[i].def_tipo == 'VACUO'):
                    print('Error')
        ...
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listParam)):
                if(general.listFuncion[i].listParam[j].ID == str(expresion_simple.expresion_simple_p)):
                    if (general.listFuncion[i].listParam[j].def_tipo == 'ENT'):
                        if(general.listFuncion[i].listParam[j].esArreglo == False):
                            print('Variable ' + str(expresion_simple.expresion_simple_p) + ' declarada como Parametro')
                        elif(general.listFuncion[i].listParam[j].esArreglo == True):
                            print('Error')
                    elif (general.listFuncion[i].listParam[j].def_tipo == 'VACUO'):
                        print('Error')
        ...
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listLocales)):
                if (general.listFuncion[i].listLocales[j].ID == str(expresion_simple.expresion_simple_p)):
                    if (general.listFuncion[i].listLocales[j].def_tipo == 'ENT'):
                        if(general.listFuncion[i].listLocales[j].esArreglo == False):
                            print('Variable ' + str(expresion_simple.expresion_simple_p) + ' declarada como Local')
                        elif(general.listFuncion[i].listLocales[j].esArreglo == True):
                            print('Error')
                    elif (general.listFuncion[i].listLocales[j].def_tipo == 'VACUO'):
                        print('Error')
        ...
        # Verificar si variable está en Globales
        for i in range(len(general.listGlobales)):
            if (general.listGlobales[i].ID == str(expresion_simple.expresion_aditiva_p)):
                if (general.listGlobales[i].def_tipo == 'ENT'):
                    if (general.listGlobales[i].esArreglo == False):
                        print('Variable ' + str(expresion_simple.expresion_aditiva_p) + ' declarada como Global')
                    elif (general.listGlobales[i].esArreglo == True):
                        print('Error')
                elif (general.listGlobales[i].def_tipo == 'VACUO'):
                    print('Error')
        ...
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listParam)):
                if (general.listFuncion[i].listParam[j].ID == str(expresion_simple.expresion_aditiva_p)):
                    if (general.listFuncion[i].listParam[j].def_tipo == 'ENT'):
                        if(general.listFuncion[i].listParam[j].esArreglo == False):
                            print('Variable ' + str(expresion_simple.expresion_aditiva_p) + ' declarada como Parametro')
                        elif(general.listFuncion[i].listParam[j].esArreglo == True):
                            print('Error')
                    elif (general.listFuncion[i].listParam[j].def_tipo == 'VACUO'):
                        print('Error')
        ...
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listLocales)):
                if (general.listFuncion[i].listLocales[j].ID == str(expresion_simple.expresion_aditiva_p)):
                    if (general.listFuncion[i].listLocales[j].def_tipo == 'ENT'):
                        if(general.listFuncion[i].listLocales[j].esArreglo == False):
                            print('Variable ' + str(expresion_simple.expresion_aditiva_p) + ' declarada como Local')
                        elif(general.listFuncion[i].listLocales[j].esArreglo == True):
                            print('Error')
                    elif (general.listFuncion[i].listLocales[j].def_tipo == 'VACUO'):
                        print('Error')

        if (str(expresion_simple.expresion_aditiva_p).isnumeric()):
            print(str(expresion_simple.expresion_aditiva_p))

        print(str(expresion_simple.expresion_simple_p) + "  " + str(expresion_simple.relop_p) + "  "  + str(expresion_simple.expresion_aditiva_p))
    ...
    def visit_expresion_aditiva(self, expresion_aditiva):
        ...
        #Verificar si variable está en Globales
        for i in range (len(general.listGlobales)):
            if(general.listGlobales[i].ID == str(expresion_aditiva.expresion_aditiva_p)):
                if(general.listGlobales[i].def_tipo == 'ENT'):
                    if (general.listGlobales[i].esArreglo == False):
                        print('Variable ' + str(expresion_aditiva.expresion_aditiva_p) + ' declarada como Global')
                    elif (general.listGlobales[i].esArreglo == True):
                        print('Error')
                elif (general.listGlobales[i].def_tipo == 'VACUO'):
                    print('Error')
        ...
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listParam)):
                if(general.listFuncion[i].listParam[j].ID == str(expresion_aditiva.expresion_aditiva_p)):
                    if (general.listFuncion[i].listParam[j].def_tipo == 'ENT'):
                        if(general.listFuncion[i].listParam[j].esArreglo == False):
                            print('Variable ' + str(expresion_aditiva.expresion_aditiva_p) + ' declarada como Parametro')
                        elif(general.listFuncion[i].listParam[j].esArreglo == True):
                            print('Error')
                    elif (general.listFuncion[i].listParam[j].def_tipo == 'VACUO'):
                        print('Error')
        ...
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listLocales)):
                if (general.listFuncion[i].listLocales[j].ID == str(expresion_aditiva.expresion_aditiva_p)):
                    if (general.listFuncion[i].listLocales[j].def_tipo == 'ENT'):
                        if(general.listFuncion[i].listLocales[j].esArreglo == False):
                            print('Variable ' + str(expresion_aditiva.expresion_aditiva_p) + ' declarada como Local')
                        elif(general.listFuncion[i].listLocales[j].esArreglo == True):
                            print('Error')
                    elif (general.listFuncion[i].listLocales[j].def_tipo == 'VACUO'):
                        print('Error')
        ...
        # Verificar si variable está en Globales
        for i in range(len(general.listGlobales)):
            if (general.listGlobales[i].ID == str(expresion_aditiva.term_p)):
                if (general.listGlobales[i].def_tipo == 'ENT'):
                    if (general.listGlobales[i].esArreglo == False):
                        print('Variable ' + str(expresion_aditiva.term_p) + ' declarada como Global')
                    elif (general.listGlobales[i].esArreglo == True):
                        print('Error')
                elif (general.listGlobales[i].def_tipo == 'VACUO'):
                    print('Error')
        ...
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listParam)):
                if (general.listFuncion[i].listParam[j].ID == str(expresion_aditiva.term_p)):
                    if (general.listFuncion[i].listParam[j].def_tipo == 'ENT'):
                        if(general.listFuncion[i].listParam[j].esArreglo == False):
                            print('Variable ' + str(expresion_aditiva.term_p) + ' declarada como Parametro')
                        elif(general.listFuncion[i].listParam[j].esArreglo == True):
                            print('Error')
                    elif (general.listFuncion[i].listParam[j].def_tipo == 'VACUO'):
                        print('Error')
        ...
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listLocales)):
                if (general.listFuncion[i].listLocales[j].ID == str(expresion_aditiva.term_p)):
                    if (general.listFuncion[i].listLocales[j].def_tipo == 'ENT'):
                        if(general.listFuncion[i].listLocales[j].esArreglo == False):
                            print('Variable ' + str(expresion_aditiva.term_p) + ' declarada como Local')
                        elif(general.listFuncion[i].listLocales[j].esArreglo == True):
                            print('Error')
                    elif (general.listFuncion[i].listLocales[j].def_tipo == 'VACUO'):
                        print('Error')

        if (str(expresion_aditiva.term_p).isnumeric()):
            print(str(expresion_aditiva.term_p))
    ...
    def visit_term(self, term):
        ...
        #Verificar si variable está en Globales
        for i in range (len(general.listGlobales)):
            if(general.listGlobales[i].ID == str(term.term_p)):
                if(general.listGlobales[i].def_tipo == 'ENT'):
                    if (general.listGlobales[i].esArreglo == False):
                        print('Variable ' + str(term.term_p) + ' declarada como Global')
                    elif (general.listGlobales[i].esArreglo == True):
                        print('Error')
                elif (general.listGlobales[i].def_tipo == 'VACUO'):
                    print('Error')
        ...
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listParam)):
                if(general.listFuncion[i].listParam[j].ID == str(term.term_p)):
                    if (general.listFuncion[i].listParam[j].def_tipo == 'ENT'):
                        if(general.listFuncion[i].listParam[j].esArreglo == False):
                            print('Variable ' + str(term.term_p) + ' declarada como Parametro')
                        elif(general.listFuncion[i].listParam[j].esArreglo == True):
                            print('Error')
                    elif (general.listFuncion[i].listParam[j].def_tipo == 'VACUO'):
                        print('Error')
        ...
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listLocales)):
                if (general.listFuncion[i].listLocales[j].ID == str(term.term_p)):
                    if (general.listFuncion[i].listLocales[j].def_tipo == 'ENT'):
                        if(general.listFuncion[i].listLocales[j].esArreglo == False):
                            print('Variable ' + str(term.term_p) + ' declarada como Local')
                        elif(general.listFuncion[i].listLocales[j].esArreglo == True):
                            print('Error')
                    elif (general.listFuncion[i].listLocales[j].def_tipo == 'VACUO'):
                        print('Error')
        ...
        # Verificar si variable está en Globales
        for i in range(len(general.listGlobales)):
            if (general.listGlobales[i].ID == str(term.factor_p)):
                if (general.listGlobales[i].def_tipo == 'ENT'):
                    if (general.listGlobales[i].esArreglo == False):
                        print('Variable ' + str(term.factor_p) + ' declarada como Global')
                    elif (general.listGlobales[i].esArreglo == True):
                        print('Error')
                elif (general.listGlobales[i].def_tipo == 'VACUO'):
                    print('Error')
        ...
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listParam)):
                if (general.listFuncion[i].listParam[j].ID == str(term.factor_p)):
                    if (general.listFuncion[i].listParam[j].def_tipo == 'ENT'):
                        if(general.listFuncion[i].listParam[j].esArreglo == False):
                            print('Variable ' + str(term.factor_p) + ' declarada como Parametro')
                        elif(general.listFuncion[i].listParam[j].esArreglo == True):
                            print('Error')
                    elif (general.listFuncion[i].listParam[j].def_tipo == 'VACUO'):
                        print('Error')
        ...
        for i in range(len(general.listFuncion)):
            for j in range(len(general.listFuncion[i].listLocales)):
                if (general.listFuncion[i].listLocales[j].ID == str(term.factor_p)):
                    if (general.listFuncion[i].listLocales[j].def_tipo == 'ENT'):
                        if(general.listFuncion[i].listLocales[j].esArreglo == False):
                            print('Variable ' + str(term.factor_p) + ' declarada como Local')
                        elif(general.listFuncion[i].listLocales[j].esArreglo == True):
                            print('Error')
                    elif (general.listFuncion[i].listLocales[j].def_tipo == 'VACUO'):
                        print('Error')

        if (str(term.factor_p).isnumeric()):
            print(str(term.factor_p))
